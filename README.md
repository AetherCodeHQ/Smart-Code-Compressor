...
> Smart-Code-Compressor - Open source tool by AetherCodeHQ

`cli` `code` `compression` `golang` `optimization`

## What is it?

**Smart Code Compressor** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Smart-Code-Compressor.git
cd Smart-Code-Compressor
go build -o Smart-Code-Compressor .
```

### Run
```bash
./Smart-Code-Compressor --help
```
...
```bash
# Basic usage
./Smart-Code-Compressor --path ./target

# With options
./Smart-Code-Compressor --path ./target --format json --output report.json

# Verbose mode
./Smart-Code-Compressor --path ./target --verbose
```
...
### Basic scan
```bash
./Smart-Code-Compressor --path ./my-project
```

### JSON report
```bash
./Smart-Code-Compressor --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Smart Code Compressor
  run: ./Smart-Code-Compressor --path . --format json --output report.json
```
